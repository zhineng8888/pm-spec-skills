...
给HTML原型页面注入「文档模式」系统，支持左侧页面预览 + 右侧双Tab（标注说明/页面说明） + 页面标注编辑功能。
...
**标注数据以 `[页面名]标注.json` 文件形式存放在 HTML 所在目录。标注说明以 `[页面名]标注.md` 文件形式存放。**
...
模块目录/
  页面名.html                 # 页面文件
  页面名.md                   # 产品说明文档（右侧"页面说明"Tab展示）
  页面名标注.json             # 标注数据（自动保存/加载，带缓存绕过）
  页面名标注.md               # 标注说明文档（右侧"标注"Tab展示）
shared/
...
| 📖 文档模式 | 左侧页面预览（iframe）+ 右侧双Tab（标注说明/页面说明） |
...
| 💬 标注说明 | 同目录 `[页面名]标注.md`，hover整框弹出tooltip，左侧方形编号 + "说明"标签 |
...
- 标注说明从 `页面名标注.md` 加载，右侧"标注说明" Tab 展示
...
| 场景 | Tab栏 | 默认展示 |
|------|--------|----------|
| 标注MD + 页面MD 都存在 | 显示双Tab | 标注说明（优先） |
| 只有标注说明MD | 隐藏Tab栏 | 直接展示标注说明内容 |
| 只有页面说明MD | 隐藏Tab栏 | 直接展示页面说明内容 |
| 都没有 | 显示Tab栏 | 内容为空提示 |

**实现逻辑：** `enterDocMode()` 中 `Promise.all([loadMdDoc(), loadAnnoDoc()])` 加载完成后调用 `updateTabsVisibility()`，根据 `_mdFileExists` 和 `_annoMdFileExists` 两个状态标志决定Tab显隐和默认激活项。
...
- 标注 JSON/MD 也是同目录、`页面名标注.json` / `页面名标注.md`
...
- 如果页面只有页面说明MD（无标注MD），右侧Tab栏自动隐藏，直接展示内容
